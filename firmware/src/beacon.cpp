...
// and the next ignition just boots it fresh (~2 s to advertising, well inside
// even a door-open blip). Reflash over USB in the car or on the bench.
//
...
// a name-carrying non-connectable packet every 100 ms at +9 dBm. The 2026-08-29
...
void loop() {
  ...
  if (millis() - lastHb >= 60u * 1000u) {
    lastHb = millis();
    Serial.printf("up %lus, advertising '%s' @100ms +9dBm, ", millis() / 1000, BEACON_NAME);
    printMac();
  }
  delay(2950);
}
